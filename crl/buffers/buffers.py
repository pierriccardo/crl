...
class SimpleTrajBuffer:
    def __init__(self, capacity_episodes: int, device="cpu"):
    ...
    def add_episode(self, ep: dict):
        # ep must contain tensors on device (or convertible):
        # "s": [T,s_dim], "a": [T,a_dim], "r": [T,1], "d":[T,1], "sp":[T,s_dim]
        if len(self.eps) < self.capacity:
            self.eps.append(ep)
        else:
            self.eps[self.idx] = ep
            self.idx = (self.idx + 1) % self.capacity

    def __len__(self):
        return len(self.eps)

    def sample(self, B: int, L: int, burnin: int = 0):
        if len(self.eps) == 0:
            raise ValueError("Empty buffer")
        K = burnin
        need = K + L
        max_T = max(ep["s"].shape[0] for ep in self.eps)
        if max_T < need:
            raise ValueError(
                f"VAE sequence length (burnin+L={need}) exceeds max episode length ({max_T}). "
                f"Set vae_seq_length <= max_episode_steps (e.g. vae_seq_length=-1 to auto-set)."
            )
        out = {k: [] for k in ["s", "a", "r", "d", "sp"]}
        max_resample = 10_000

        for _ in range(B):
            ep = random.choice(self.eps)
            T = ep["s"].shape[0]
            attempts = 0
            while T < need and attempts < max_resample:
                ep = random.choice(self.eps)
                T = ep["s"].shape[0]
                attempts += 1
            if T < need:
                raise ValueError(
                    f"No episode with length >= {need} found after {max_resample} samples "
                    f"(max episode length={max_T}). Set vae_seq_length <= max_episode_steps."
                )

            t0 = random.randint(0, T - need)
            sl = slice(t0, t0 + need)
        ...
        # stack: [B, K+L, ...]
        for k in out:
            out[k] = torch.stack(out[k], dim=0).to(self.device)
