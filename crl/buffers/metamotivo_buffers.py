...
def _to_torch(value: Any, device: Device | None = None) -> Any:
    raise Exception(
        f"No known conversion for type ({type(value)}) to PyTorch registered. Report as issue on github."
    )
...
class DictBuffer:
    ...
    def extend(self, data: Dict) -> None:
        if self.storage is None:
            ...
            self._non_nested_key = [
                k for k, v in self.storage.items() if not isinstance(v, Mapping)
            ][0]
            ...
            for k, v in data.items():
                if isinstance(v, Mapping):
                    ...
                    if v.shape[0] != expected_dim:
                        raise ValueError(
                            "We expect all keys to have the same dimension"
                        )
                    end = self._idx + v.shape[0]
                    if end >= self.capacity:
                        # Wrap data
                        diff = self.capacity - self._idx
                        storage[k][self._idx :] = _to_torch(
                            v[:diff], device=self.device
                        )
                        storage[k][: v.shape[0] - diff] = _to_torch(
                            v[diff:], device=self.device
                        )
                        self._is_full = True
                    else:
...
class TrajectoryBuffer:
    ...
    output_key_tp1: list[str] = dataclasses.field(
    ...
    def extend(self, data: List[dict]) -> None:
        if self.storage is None:
            ...
            self._is_full = False
            self.priorities = (
                torch.ones(self.capacity, device=self.device, dtype=torch.float32)
                / self.capacity
            )
    ...
    def sample(self, batch_size: int = 1) -> Dict[str, torch.Tensor]:
        ...
        for ep_idx in self.ep_ind:
            ...
            if _ep is None:
                raise ValueError(
                    f"Attempted to sample from unfilled slot {ep_idx.item()}"
                )
            length = _ep[self.output_key_t[0]].shape[0]
            min_length = self.seq_length + offset
            if length < min_length:
                raise ValueError(
                    f"Episode at index {ep_idx.item()} has length {length}, "
                    f"but need at least {min_length} (seq_length={self.seq_length}, "
                    f"offset={offset})"
                )
            ...
            for k in self.output_key_tp1:
                output["next"][k].append(
                    _ep[k][time_idx + offset : time_idx + offset + self.seq_length]
                )
...
def initialize_storage(
    data: Dict, storage: Dict, capacity: int, device: Device
) -> None:
...
class ZBuffer:
    ...
    def sample(self, num, device=None) -> torch.Tensor:
        ...
        return (
            self._storage[idx].clone().to(device if device is not None else self.device)
        )
